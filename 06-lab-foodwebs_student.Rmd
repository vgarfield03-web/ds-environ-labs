...
probEat <- function(pred_mass, prey_mass, alpha, beta, gamma) {
  round(inv_logit(alpha + beta * log(pred_mass / prey_mass)+ 
                    gamma * (log(pred_mass / prey_mass))^2), 2)
}
```
...
```{r}
# who eats whom, female species
n <- nrow(sp_mass)
prob_matrix <- matrix(0, n, n)
rownames(prob_matrix) <- rownames(sp_mass)
colnames(prob_matrix) <- rownames(sp_mass)

for(i in 1:n){
  for(j in 1:n){
    prob_matrix[i,j] <- probEat(sp_mass[i,1], sp_mass[j,1], 2.51, 0.79, -0.37)
  }
}
head(prob_matrix)
```
...
carnivores <- c(rep(1, 9), rep(0, length(row_labs_sp)- 9))
# your code here
test <- sweep(prob_matrix, MARGIN=2,carnivores, `*`)
```
...
time2 <- sp_occ[, 2]
sum(time2)
web_time2 <- sweep(test, MARGIN = 1, time2, `*`)
web_time2 <- sweep(web_time2, MARGIN = 2, time2, `*`)

library(bipartite)
web_time2 <- empty(web_time2)
...
###  your code here
n_time <- ncol(sp_occ)
web3 <- vector("list", n_time)

for (i in 1:n_time) {
  time_i <- sp_occ[, i]                       # 
  
  web_i <- sweep(test, MARGIN = 1, time_i, `*`)  # 
  
  web_i <- sweep(web_i, MARGIN = 2, time_i, `*`)  # 
...
# Show first 5 rows and columns of time period 5
web3[[5]][1:5, 1:5]
...
common_species <- intersect(rownames(web_time2), colnames(web_time2))
web_square <- web_time2[common_species, common_species, drop = FALSE]

igraph_list <- graph_from_adjacency_matrix(
  web_square,
   mode = "directed",
      diag = FALSE,
        weighted = TRUE)
...
species <- intersect(rownames(web_matrix), colnames(web_matrix))
web_matrix <- web_matrix[species,species]

  
  ###  convert to a network 3d object
g <- graph_from_adjacency_matrix(web_matrix, mode = "directed", weighted = TRUE, diag = FALSE)

col <- ifelse(carnivores[species] == 1, "red", "blue")
  ###  plot the network
  plot(g, vertex.color = col, main = paste("Food Web - Time", i))
}
```
