...
co2_sub <- co2 %>%
  select(decimal_date, year, month, average) %>%
  rename(avg_ppm = average)
...
co2_top <- co2_sub %>%
  filter(year >= 2000, !is.na(avg_ppm)) %>%
  arrange(dplyr::desc(avg_ppm)) %>%
  slice_head(n = 5)
co2_top
```
...
co2_enriched %>%
  filter(!is.na(delta)) %>%
  ggplot(ggplot2::aes(delta)) +
  geom_histogram(bins = 10) +
  facet_wrap(~ decade) +
  labs(x = "Δ ppm (current − previous month)", y = "Months",
...
```{r}
co2_long %>%
  ggplot(aes(x=year, y=ppmv, color = series)) +
  geom_line()
