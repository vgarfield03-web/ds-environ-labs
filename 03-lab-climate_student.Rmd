---
---
title: "03-Lab Climate"
author: "Tori Garfield"
output: pdf_document
...
```{r, render=print}
co2.test2 <- read.csv("data/co2_mm_mlo.txt",
                 comment = "#")
head(co2.test2)
```
...
```{r}
readr::read_table
co2 <- read_table(co2_path, comment = "#",col_names = c("year", "month", "decimal_date", 
"average", "interpolated", "trend", "stdev_days", "uncertainty_average"),
col_types = c("iidddidd"), na = c("-1", "-99.99", "-9.99", "-0.99"))
...
Each of the last years has consecutively set new records on global climate.  In this section we will analyze global mean temperature data. [Data from](http://climate.nasa.gov/vital-signs/global-temperature)
 ...
 -1a. Describe the data set. You can do additional searches to learn more about the   -   
-- ANSWER: ...
- data given the description on the data page is not detailed. 
This data set is the changes in global surface temperature compared to the baseline average between 1951-1980. 
- 1b. Describe what kind of column each data contains and what units it is measured in. 
ANSWER: 
columns are years and temperature anomaly(C).
...
ANSWER: ...
- single thermometer, and grid boxes around the world, satellite measurements. average the two hemisphereic values 
- measure of uncertainty is +/- 0.15 C
-1d.  What is the resolution of the data?
ANSWER: ...
resolution of this data is 2 degree x 2 degree, latitude-logitude 1
-1e. Are their missing values? How should they be handled?
ANSWER: ... 
missing values, they use GISTEMP based on spatial and temporal relationships, using zonal bands and propagating for uncertainty 
#### Question 2:
...
temp <- read_table("https://data.giss.nasa.gov/gistemp/graphs/graph_data/Global_Mean_Estimates_based_on_Land_and_Ocean_Data/graph.txt",
                   col_types = "idd", skip = 5, na = "*", col_names = c("Year", "yearly_Mean", "5-year_average"))
temp
```
...
## your code here
temp_long <- temp %>%
  select(Year, yearly_Mean, `5-year_average`) %>%
  pivot_longer(
    c(yearly_Mean, `5-year_average`),
    names_to = "series",
    values_to = "ppmv"
  )
...
head(temp_yearly_wide)

library(dplyr)
library(tidyr)

temp_ym <- temp %>%
  transmute(Year, avg_ppm = yearly_Mean) %>%
  unite("ym", Year, sep = "-", remove = FALSE)


head(temp_ym)

temp_ym_split <- temp_ym %>%
...
temp %>% 
  ggplot2::ggplot(ggplot2::aes(Year, yearly_Mean)) +
  ggplot2::geom_line() + 
  ggplot2::labs(x = "Year", y = "temp (ppm)", title = "Rise in Temperature") +
...
ANSWER: ...
5 year average is the average of multi-year  mean value, annual is the yearly average
...
### your code here
temp %>%
 mutate(annual = RcppRoll::roll_mean(yearly_Mean,
                                     n=12L,
                                     align = "left",
                                     fill = NA,
                                     na.rm=TRUE,
...
temp <- temp %>%
  mutate(
  avg_5_year = RcppRoll::roll_mean(yearly_Mean, n = 5L, align = "center", na.rm = TRUE, fill = NA),
    avg_10_year = RcppRoll::roll_mean(yearly_Mean, n = 10L, align = "center", na.rm = TRUE, fill = NA),
    avg_20_year = RcppRoll::roll_mean(yearly_Mean, n = 20L, align = "center", na.rm = TRUE, fill = NA)
  )
temp_long <- temp %>%
  tidyr::pivot_longer(
...
ggplot(temp, aes(x = Year)) +
  geom_line(aes(y = yearly_Mean, color = "Annual"), alpha = 0.5) +
  geom_line(aes(y = avg_5_year, color = "5-Year Average"), size = 1) +
  geom_line(aes(y = avg_10_year, color = "10-Year Average"), size = 1) +
  geom_line(aes(y = avg_20_year, color = "20-Year Average"), size = 1) +
  scale_color_manual(values = c("Annual" = "gray", "5-Year Average" = "blue", "10-Year Average" = "orange", "20-Year Average" = "red")) +
  labs(
    title = "Comparison of Moving Averages",
    subtitle = "Annual data smoothed over different time periods",
    y = "Temperature",
    color = "Series"
  ) +
  theme_minimal()
...
ANSWER: ...
year, Antartica Mass (Gt) units are gigatons, and units are metric tons. numbers come from a measure of earths glacier ice sheets and ice melting, draining into the ocean 

- 6b.  What is the uncertainty in measurement? Resolution of the data? Interpretation of missing values?

ANSWER: 
- Uncertainty: There is a period of time where data was not being collected, this creates a potentially fallacy in the data, what may we have missed here
- Resolution: outside of period of missing data, seems pretty consistent.
- Missing data: there is a period of time where data was not being collected, this creates a potentially fallacy in the data, what may we have missed here
...
### your code
library(tidyverse)
my_data <- read.csv("../labs/data/499_GRN_ANT_mass_changes.csv", skip = 9)


colnames(my_data) <- c("TIME", "Greenland", "Antarctica")


tidy_data <- my_data %>%
  pivot_longer(
    cols = c("Greenland", "Antarctica"),
    names_to = "Region",
...
```{r}
### your code
ggplot(tidy_data, aes(x = TIME, y = Mass_Change, color = Region)) +
  geom_point() +
  geom_line() +
  facet_wrap(~ Region, ncol = 1, scales = "free_y") +
  labs(
    title = "Mass Lost Over Time: Greenland and Antarctica",
    x = "Time",
    y = "Mass Change (Gt)"
  ) +
  theme_minimal()
```
ANSWER: ...
steady decreases, greenland has more recoveries and then regressions steadily than antartica. anarctica has spikes and drops, with a period of stagnance. 
Isn't this fun?! 
...
ANSWER:
year, sea height, units are (mm)

- 7b. Where do these data come from (try googling if the description isn't satisfactory)? 
ANSWER: 
this data is found thorugh satellite images, and tide gauges. 

- 7c. What is the uncertainty in measurement? Resolution of the data? Interpretation of missing values?
ANSWER:  
- Uncertainty: unpredictable ice sheet behavior, and the complexity of climate 
- Resolution: is the level of detail in said data set, which for this data set is quite well done, I think highly recorded.  
-Missing data: can lead to incorrect conclusions drawn from analysis that is not completely filled in.
...
library(tidyverse)
my_data <- read.table(
  "../labs/data/647_Global_Temperature_Data_File.txt",
  skip = 2,
  header = TRUE
)

colnames(my_data) <- c("Year", "No_Smoothing", "Lowess_5")
...
tidy_data1 <- my_data %>%
  pivot_longer(
    cols = c("No_Smoothing", "Lowess_5"),
    names_to = "Sea_Height",
    values_to = "height_increase"
  )
...
```{r}
### your code
ggplot(tidy_data1, aes(x = Year, y = height_increase, color = Sea_Height)) +
  geom_point() +
  geom_line() +
  labs(
...
ANSWER: ...
steady increase of water levels rising, drastic dips and decreases in terms of smoothness. Seems to me, that steady has become unpredictable maybe. 
...
- if we look at our c02 grah we can see in may (the highest month), our count is at 60+, when we move onto september(the lower/lowest), we see that our count is between 25 and 40. 
- What month is the maximum? What is the minimum?
The maximum is around the spring time, as we get closer to summer (mayish), then the lowest point is during september
- What do you think could explain the seasonal cycle observed here?
this is impacted by the seasons changing, trees losing their leaves, there is a slow emission of co2 via these plants, in conjunction with human emissions, we hit our peak around spring, right before these plants come back full force. 
- 
